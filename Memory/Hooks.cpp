...
void Hooks::Init() {
	...
	// Vtables
	{
		...
		// LocalPlayer::vtable
		{
			...
			if (localPlayerVtable == 0x0 || sigOffset == 0x0)
				...
				g_Hooks.JumpPowerHook = std::make_unique<FuncHook>(localPlayerVtable[357], Hooks::JumpPower);//cant find
	...
	// Signatures
	{
		...
		void* _renderText = reinterpret_cast<void*>(FindSignature("48 8B ? 48 89 ? ? 55 56 57 41 ? 41 ? 41 ? 41 ? 48 8D ? ? ? ? ? 48 81 EC ? ? ? ? 0F 29 ? ? 0F 29 ? ? 48 8B ? ? ? ? ? 48 33 ? 48 89 ? ? ? ? ? 4C 8B ? 48 89 ? ? ? 4C 8B"));
...
void Hooks::Actor_lerpMotion(C_Entity* _this, vec3_t motVec) {
	...
	if (noKnockbackmod->isEnabled()) {
		...
		if (!networkSender)
			networkSender = reinterpret_cast<void*>(9 + FindSignature("48 8B CB FF ?? ?? ?? ?? 00 C6 47 ?? 01 48 8B 5C 24"));//89 81 ? ? ? ? 8B 42 ? 89 81 ? ? ? ? 8B 42 ? 89 81 ? ? ? ? C3 CC CC CC CC CC 48 89 5C 24
