...
void DrawUtils::draw2D(C_Entity* ent, float lineWidth) {
	...
	corners[0] = vec3_t(base.x - ent->width / 1.5f * -sin(ofs), base.y + ent->height, base.z - ent->width / 1.5f * cos(ofs));
	corners[1] = vec3_t(base.x + ent->width / 1.5f * -sin(ofs), base.y + ent->height, base.z + ent->width / 1.5f * cos(ofs));
	corners[2] = vec3_t(base.x - ent->width / 1.5f * -sin(ofs), base.y, base.z - ent->width / 1.5f * cos(ofs));
	corners[3] = vec3_t(base.x + ent->width / 1.5f * -sin(ofs), base.y, base.z + ent->width / 1.5f * cos(ofs));
