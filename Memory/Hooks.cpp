...
void Hooks::ChatScreenController_sendChatMessage(uint8_t* _this) {
	...
	if (*textLength) {
		...
		// (THIS LINE IS CAUSING A CRASH WHEN USING ANY COMMAND SO THIS IS A TEMP FIX)	addCommandToChatHistory(a1, (char*)(_this + 0xA70));  // This will put the command in the chat history (Arrow up/down)
